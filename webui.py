...
def inpaint_mode_change(mode, inpaint_engine_version):
    assert mode in flags.inpaint_options

    # inpaint_additional_prompt, outpaint_selections, example_inpaint_prompts,
    # inpaint_disable_initial_latent, inpaint_engine,
    # inpaint_strength, inpaint_respective_field

    if mode == flags.inpaint_option_detail:
        return [
    ...
    if mode == flags.inpaint_option_modify:
        return [
            gr.update(visible=True), gr.update(visible=False, value=[]),
...
with shared.gradio_root:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=3, min_width=0):
                    ...
                    stop_button.click(stop_clicked, inputs=currentTask, outputs=currentTask, queue=False, show_progress='hidden', _js='cancelGenerateForever')
                    ...
                    with gr.Tab(label='Inpaint or Outpaint', id='inpaint_tab') as inpaint_tab:
                        with gr.Row():
                            with gr.Column():
                                ...
                                inpaint_mode = gr.Dropdown(choices=flags.inpaint_options, value=modules.config.default_inpaint_method, label='Method')
                        ...
                        with gr.Row():
                            ...
                            with gr.Column():
                                ...
                                def trigger_show_image_properties(image):
                                    value = modules.util.get_image_size_info(image, flags.sdxl_aspect_ratios)
                    ...
                    for index in range(modules.config.default_enhance_tabs):
                        with gr.Tab(label=f'#{index + 1}') as enhance_tab_item:
                            ...
                            with gr.Accordion("Inpaint", visible=True, open=False):
                                enhance_inpaint_mode = gr.Dropdown(choices=flags.inpaint_options,
            ...
            input_image_checkbox.change(lambda x: gr.update(visible=x), inputs=input_image_checkbox,
                                        outputs=image_input_panel, queue=False, show_progress='hidden', _js=switch_js)
            ip_advanced.change(lambda: None, queue=False, show_progress='hidden', _js=down_js)

            current_tab = gr.Textbox(value='uov', visible=False)
            uov_tab.select(lambda: 'uov', outputs=current_tab, queue=False, _js=down_js, show_progress='hidden')
            inpaint_tab.select(lambda: 'inpaint', outputs=current_tab, queue=False, _js=down_js, show_progress='hidden')
            ip_tab.select(lambda: 'ip', outputs=current_tab, queue=False, _js=down_js, show_progress='hidden')
            describe_tab.select(lambda: 'desc', outputs=current_tab, queue=False, _js=down_js, show_progress='hidden')
            enhance_tab.select(lambda: 'enhance', outputs=current_tab, queue=False, _js=down_js, show_progress='hidden')
            metadata_tab.select(lambda: 'metadata', outputs=current_tab, queue=False, _js=down_js, show_progress='hidden')
            enhance_checkbox.change(lambda x: gr.update(visible=x), inputs=enhance_checkbox,
                                        outputs=enhance_input_panel, queue=False, show_progress='hidden', _js=switch_js)
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            with gr.Tab(label='Settings'):
                ...
                with gr.Accordion(label='Aspect Ratios', open=False, elem_id='aspect_ratios_accordion') as aspect_ratios_accordion:
                    ...
                    aspect_ratios_selection.change(lambda x: None, inputs=aspect_ratios_selection, queue=False, show_progress='hidden', _js='(x)=>{refresh_aspect_ratios_label(x);}')
                    shared.gradio_root.load(lambda x: None, inputs=aspect_ratios_selection, queue=False, show_progress='hidden', _js='(x)=>{refresh_aspect_ratios_label(x);}')
            ...
            with gr.Tab(label='Advanced'):
                ...
                with gr.Column(visible=modules.config.default_developer_debug_mode_checkbox) as dev_tools:
                    with gr.Tab(label='Debug Tools'):
                        ...
                        vae_name = gr.Dropdown(label='VAE', choices=[flags.default_vae] + modules.config.vae_filenames,
        ...
        advanced_checkbox.change(lambda x: gr.update(visible=x), advanced_checkbox, advanced_column,
                                 queue=False, show_progress='hidden') \
            .then(fn=lambda: None, _js='refresh_grid_delayed', queue=False, show_progress='hidden')
